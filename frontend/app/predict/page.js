...
export default function Home() {
  ...
  let teams = searchParams.get("teams");
  console.log(teams);
  if (teams != null) {
    teams = JSON.parse(teams);
  }
  const [left_team, set_left_team] = useState(teams ? teams[0] : null);
  const [right_team, set_right_team] = useState(teams ? teams[1] : null);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-5">
      ...
      <div className="flex flex-row justify-center text-middle items-center w-1/2">
        {prediction ? (
